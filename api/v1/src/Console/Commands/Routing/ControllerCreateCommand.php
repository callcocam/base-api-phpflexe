...
class ControllerCreateCommand extends Command
{
    ...
    protected function getModuleData($moduleName) {
        ...
        foreach ($listModules as $module) {
            if (strtoupper($module::NAME) == strtoupper($moduleName)) {
                ...
                $arrayReturn['name-space-module'] = sprintf( '%s\Controller',$module::getNamespace());
    ...
    protected function writeController() {
        ...
        $io = new SymfonyStyle($this->input, $this->output);
        $modelStub = file_get_contents(__DIR__ . '/snippets/controller.model.snippets');
        ...
        $modelStub = str_replace('{{ROUTE-NAME}}', strtolower($this->moduleName), $modelStub);
        $modelStub = str_replace('{{CONTROLLER}}', strtolower($this->nameController), $modelStub);

        $controller = sprintf( '%s/%sController.php',$this->pathWriteModule, $this->nameController);
