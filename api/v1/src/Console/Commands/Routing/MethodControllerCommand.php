...
class MethodControllerCommand extends Command
{
    ...
    private function createReflactionClass() {
        try {
           $this->reflectionClass = new \ReflectionClass(sprintf( '%s\\Controller\\%sController',$this->metaModule::getNamespace(), $this->nameController));
    ...
    private function validateController() {
        $this->pathFullController = sprintf('%s/%sController.php', $this->pathController , $this->nameController);
    ...
    private function createStub() {
        $fileStub = file_get_contents(__DIR__ . '/snippets/method.model.snippets');
